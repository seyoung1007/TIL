...
/* ─────────────────────────
   CHIP TOGGLE (트레이너 페이지용 - AI 위저드 칩은 ai.js에서 처리)
───────────────────────── */
document.addEventListener('click', e => {
  // AI 위저드 내부 칩은 ai.js에서 처리
  if (e.target.closest('.ai-wizard-wrap')) return;
...
function initSlider() {
  ...
  function goSlide(n) {
    ...
    // 카운터 업데이트
    if (curEl) curEl.textContent = String(slideIdx + 1).padStart(2, '0');

    // 프로그레스 바 업데이트
    if (progressBar) progressBar.style.width = `${((slideIdx + 1) / SLIDES.length) * 100}%`;
  }
...
/* 예산 슬라이더 라이브 카운터 */
document.getElementById('bslider')?.addEventListener('input', e => onBudgetChange(e.target.value));
